...
export default function Todolist() {
  ...
  async function fetchTasks() {
    ...
    if (error) {
      ...
      setTasks(data);
      console.log('Toutes les tâches :', data);
    }
  }
  ...
  async function handleButtonClick() {
    if (task) {
      ...
      if (error) {
        console.error('Erreur lors de l\'insertion de la tâche :', error.message);
      } else {
  ...
  async function handleDone(taskId) {
    ...
    if (error) {
      ...
      setTasks((prevTasks) => prevTasks.map(task => task.id === taskId ? { ...task, is_Done: true } : task));
